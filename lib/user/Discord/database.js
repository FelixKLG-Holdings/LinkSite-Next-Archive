...
async function tokenEncrypt(value) {
	return await CryptoJS.AES.encrypt(value, process.env.DB_ENCRYPTION).toString();
}
...
async function tokenDecrypt(value) {
	const byteDecrypted = await CryptoJS.AES.decrypt(value, process.env.DB_ENCRYPTION);
